...
int notify_and_recv_photos(){
	if(ring_count(peer_list) != 1){
		...
		message_photo *msg;
		do{
			int p2p_sock = connect_peer(aux->addr, aux->port);
			char buffer[MAX_SIZE];
			sprintf(buffer, "%s.%d", aux->addr, aux->port); //send addr and port
			if(!has_photos){
